...
const Contact = ({ data }) => {
  ...
  const handleFieldChange = (id, value) => {
    setContactFields(prev => prev.map(field => (field.id === id ? { ...field, value } : field)));
  };
  ...
  const addField = type => {
    const newField = {
      ...
      type,
      label: type === 'phone' ? 'Phone' : type === 'location' ? 'Location' : 'Custom Field',
      value: '',
      icon: type === 'phone' ? 'phone' : type === 'location' ? 'location' : 'portfolio',
      required: false,
      customIcon: null,
    };
  ...
  const handleSocialChange = (id, value) => {
    setSocialLinks(prev => prev.map(social => (social.id === id ? { ...social, value } : social)));
  };
  ...
  const handleIconUpload = async (fieldId, file) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setContactFields(prev =>
          prev.map(field =>
            field.id === fieldId ? { ...field, customIcon: result.url, icon: null } : field
          )
        );
  ...
  const removeCustomIcon = fieldId => {
    setContactFields(prev =>
      prev.map(field =>
        field.id === fieldId ? { ...field, customIcon: null, icon: 'portfolio' } : field
      )
    );
  };
  ...
  const handleSocialIconUpload = async (socialId, file) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setSocialLinks(prev =>
          prev.map(social =>
            social.id === socialId ? { ...social, customIcon: result.url, icon: null } : social
          )
        );
  ...
  const removeSocialCustomIcon = socialId => {
    setSocialLinks(prev =>
      prev.map(social =>
        social.id === socialId ? { ...social, customIcon: null, icon: 'web' } : social
      )
    );
  };
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast.success(
          result.message || 'Message sent successfully! Check your email for confirmation.'
        );
      } else {
        // Handle API errors
  ...
  const saveContactInfo = async (publish = false) => {
    try {
      ...
      socialLinks.forEach(social => {
        if (
          social.value.trim() &&
          ['github', 'linkedin', 'twitter', 'instagram'].includes(social.platform)
        ) {
  ...
  const displayFields = contactData
    ? buildContactFields(contactData).filter(field => field.value.trim())
    : [];
  ...
  return (
    <>
      <StyledContactSection id="contact" ref={revealContainer}>
        ...
        <p className="description">{formatTextWithBackticks(contactData?.description || '')}</p>
          ...
          <div className="modal-content">
              ...
              <div>
                ...
                >
                  ...
                  <p style={{ margin: '0 0 10px 0', fontSize: '12px', color: '#666' }}>
                    💡 <strong>Tip:</strong> Add custom fields for additional contact info like
                    Portfolio URL, Resume, Blog, etc.
                  </p>
            ...
            {activeTab === 'social' && (
              <div>
                ...
                <button type="button" className="add-field" onClick={addSocialPlatform}>
                  + Add Social Platform
                </button>
              </div>
            )}
            ...
            <div className="form-actions">
              ...
              <button type="button" className="save-draft" onClick={() => saveContactInfo(false)}>
                Save as Draft
              </button>
              <button type="button" className="publish" onClick={() => saveContactInfo(true)}>
                Publish
              </button>
            </div>
          </div>
